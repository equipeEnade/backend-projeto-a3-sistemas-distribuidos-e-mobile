...
router.get('/maisComprados', async (req, res) => {
    try {
        const compras = await compraService.getProdutosMaisComprados();
        res.json(compras);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
router.get('/getProdutosCompradosPorCliente/:id', async (req, res) => {
    const id = req.params.id;
    try {
        const compra = await compraService.getProdutosCompradosPorCliente(id);
        if (compra) {
            res.json(compra);
        } else {
...
router.get('/getInfoGastosUsuario/:id', async (req, res) => {
    const id = req.params.id;
    try {
        const compra = await compraService.getInfoGastosUsuario(id);
        if (compra) {
            res.json(compra);
        } else {
            res.status(404).json({ message: 'Usuário não encontrado' });
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if(compraCriado == false) res.status(400)
